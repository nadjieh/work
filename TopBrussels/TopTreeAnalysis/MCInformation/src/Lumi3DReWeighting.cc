#ifndef Lumi3DReWeighting_cc
#define Lumi3DReWeighting_cc
...
double Lumi3DReWeighting::weight3D( TRootEvent* event ) {
...
void Lumi3DReWeighting::weight3D_init( float ScaleFactor ) { 
  ...
  for (int jbin=1;jbin<NMCbin+1;jbin++) {       
    ...
    if(mean<0.) {
      cout << "BadInputValue : Your histogram generates MC luminosity values less than zero!  Please Check.  Terminating." << std::endl;
      exit(-1);
    }
  ...
  for (int jbin=1;jbin<NDatabin+1;jbin++) {       
    ...
    if(mean<0.) {
      std::cout << "BadInputValue : Your histogram generates Data luminosity values less than zero! Please Check.  Terminating." << std::endl;
      exit(-1);
    }
...
//  std::cout << " Writing weights to file Weight3D.root for re-use...  " << std::endl;
  ...
  TFile * outfile = new TFile(wFileName.c_str(),"RECREATE");
  WHist->Write();
  MHist->Write();
  DHist->Write();
  outfile->Write();
...
void Lumi3DReWeighting::weight3D_init( std::string WeightFileName ) { 
  ...
  if (!WHist) {
    std::cout << "HistogramNotFound : Could not find the histogram WHist in the file in the file " << WeightFileName << ". Terminating." << std::endl;
    exit(-1);
  }
...
void Lumi3DReWeighting::weight3D_init( std::string MCWeightFileName, std::string DataWeightFileName ) { 
  ...
  // Check if the histogram exists           
  if (!MHist) {
    std::cout << "HistogramNotFound : Could not find the histogram MHist in the file in the file " << MCWeightFileName << ". Terminating." << std::endl;
    exit(-1);
  }
  ...
  // Check if the histogram exists           
  if (!DHist) {
    std::cout << "HistogramNotFound : Could not find the histogram DHist in the file in the file " << DataWeightFileName << ". Terminating" << std::endl;
    exit(-1);
  }
