...
//=========  (Tue Aug 13 18:42:54 2013) by ROOT version5.32/00
   ...
   Default_allWcosTheta__3->Scale(20286.9/Default_allWcosTheta__3->Integral());
   ...
   pt->Draw();
  TH1D * h = 	Default_allWcosThetaStack->GetStack()->Last();
  cout<<h<<endl;
   TH1D *Default_allWcosTheta = new TH1D("Default_allWcosTheta","Default_allW: cos(#theta)",20,-1,1);
   for(int s = 0; s < 20; s++)
      Default_allWcosTheta->SetBinContent(s + 1, h->GetBinContent(s+1));
/*   Default_allWcosTheta->SetBinContent(1,2483.772);
   ...
   double sum = 0;
   for(int i = 0; i<20; i++){
	double diff = Default_allWcosTheta->GetBinContent(i+1) - Default_allWcosTheta__14->GetBinContent(i+1);
	cout<<"bin "<<i+1<<": "<<diff<<endl;
	sum+=diff;
   }
   cout<<sum<<"\t"<<Default_allWcosTheta->Integral()-Default_allWcosTheta__14->Integral()<<endl;
