...
int main(int argc, char** argv) {
    ...
    TH1D * bpt_W = 0;
#ifdef EMUComparison
    weights = TFile::Open("emuWeights.root");
    lpt_W = (TH1D*) weights->Get("ePt");
    bpt_W = (TH1D*) weights->Get("ebpt");
#endif /*EMUComparison*/
    ...
    TH1D * drf = new TH1D("drf", "drf", 2000, 0., 2.);
    TH1D * drb = new TH1D("drb", "drb", 2000, 0., 2.);
    TH1D * isoDelta_noDR = new TH1D("isoDelta_noDR", "isoDelta_noDR", 2000, 0., 2.);
    TH1D * isoRho_noDR = new TH1D("isoRho_noDR", "isoRho_noDR", 2000, 0., 2.);
    TH1D * isoDelta = new TH1D("isoDelta", "isoDelta", 2000, 0., 2.);
    TH1D * isoRho = new TH1D("isoRho", "isoRho", 2000, 0., 2.);
    TH2D * drb_iso = new TH2D("drb_iso", "drb_iso", 2000, 0., 2., 1000, 0., 1.);
    ...
    for (unsigned int fNumber = 0; fNumber < inputFileNames.size(); fNumber++) {
        ...
        for (int eventNumber = 0; eventNumber < myMuonTree->fChain->GetEntriesFast(); eventNumber++) {
            ...
            if (!myMuonTree->jetsForWtemplate(false, false)) {
                continue;
            }
#endif /*Wtemplate*/
            ...
            double DRB = dr.getValue();
            drb->Fill(DRB);
            dr.SetVectors(myEvent_tmp.Dmuons[0], myEvent_tmp.GPFJets[1]);
            double DRF = dr.getValue();
            drf->Fill(DRF);
            isoDelta_noDR->Fill(myMuonTree->leptonDeltaCorrectedRelIso);
            isoRho_noDR->Fill(myMuonTree->leptonRhoCorrectedRelIso);
            if (dirName == "TreesEle") {
                if (myMuonTree->leptonRhoCorrectedRelIso < 0.1)
                    continue;
                drb_iso->Fill(DRB, myMuonTree->leptonRhoCorrectedRelIso);
                if (DRF < 0.3 || DRB < 0.3)
                    continue;
            } else if (dirName == "TreesMu") {
                if (myMuonTree->leptonDeltaCorrectedRelIso < 0.12)
                    continue;
                drb_iso->Fill(DRB, myMuonTree->leptonDeltaCorrectedRelIso);
                if (DRB < 0.3)
                    continue;
            }
            isoDelta->Fill(myMuonTree->leptonDeltaCorrectedRelIso);
            isoRho->Fill(myMuonTree->leptonRhoCorrectedRelIso);
#ifdef EMUComparison
            if (dirName == "TreesEle") {
                if (!(myMuonTree->leptonDeltaCorrectedRelIso < 0.9
                        && myMuonTree->leptonDeltaCorrectedRelIso > 0.12))
                    continue;
            }
            if (dirName == "TreesMu") {
                if (!(myMuonTree->leptonRhoCorrectedRelIso < 0.9
                        && myMuonTree->leptonRhoCorrectedRelIso > 0.1))
                    continue;
                if (myMuonTree->leptonPt < 30)
                    continue;
            }
            int W_bin = lpt_W->GetXaxis()->FindBin(myMuonTree->leptonPt);
            double lptw = lpt_W->GetBinContent(W_bin);
            W_bin = bpt_W->GetXaxis()->FindBin(myMuonTree->bJetPt);
            double bptw = bpt_W->GetBinContent(W_bin);
