...
void plot(TString mufile, TString efile, TString name){
	...
	TH2D * muh = (TH2D*) muf->Get("DefaultTrue_allW/DefaultTrue_allWcosTheta2D");
	muh->SetTitle("mu+jets");
	cout<<name<<":\n\tmu+jets: "<<muh->GetCorrelationFactor()<<endl;
	TH2D * eh = (TH2D*) ef->Get("DefaultTrue_allW/DefaultTrue_allWcosTheta2D");
	eh->SetTitle("e+jets");
	cout<<"\te+jets: "<eh->GetCorrelationFactor()<<endl<<endl;
	TCanvas c;
	c.Divide(2,1);
	c.cd(1);
	muh->Draw();
...
void doJob(){
	...
	for(unsigned int i = 0; i<Enames.size(); i++){
		plot(mu+"_"+Munames[i]+"_TTBar_plots.root", e+"_"+Enames[i]+"_TTBar_plots.root", Munames[i]+"_"+Enames[i]);
	}
}
