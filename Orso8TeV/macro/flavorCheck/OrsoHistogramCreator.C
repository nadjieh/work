...
int main(int argc, char** argv) {
    ...
    if (!isStack)
        allW_finalMT->Sumw2();
    TH2D * nVtx_cosTheta = new TH2D("nVtxcosTheta", "nVtxcosTheta;cos(#theta_{l}*);N_{vtx}", 100, -1., 1., 100, 0, 100);
    if (!isStack)
        nVtx_cosTheta->Sumw2();
    TH2D * met_cosTheta = new TH2D("met_cosTheta", "met_cosTheta;cos(#theta_{l}*);MET", 100, -1., 1., 100, 0, 100);
    if (!isStack)
        met_cosTheta->Sumw2();
    TH2D * mt_cosTheta = new TH2D("mt_cosTheta", "mt_cosTheta;cos(#theta_{l}*);mt", 100, -1., 1., 300, 0, 300);
    if (!isStack)
        mt_cosTheta->Sumw2();
    TH2D * dphilmet_cosTheta = new TH2D("dphilmet_cosTheta", "dphilmet_cosTheta;cos(#theta_{l}*);#Delta#phi(l,MET)", 100, -1., 1., 400, -10, 10);
    if (!isStack)
        dphilmet_cosTheta->Sumw2();
    TH2D * lpt_cosTheta = new TH2D("lpt_cosTheta", "lpt_cosTheta;cos(#theta_{l}*);#Delta#phi(l,MET)", 100, -1., 1., 100, 0, 100);
    if (!isStack)
        lpt_cosTheta->Sumw2();
    ...
    if (!isStack)
        AntiTOPPT->Sumw2();
    TH1D * EventFlavor = new TH1D("EventFlavor", "event flavor", 4, 0., 4.);
    EventFlavor->GetXaxis()->SetTitle("event flavor");
    if (!isStack)
        EventFlavor->Sumw2();
    TH1D * hMetphi = 0;
    TH2D * hMetXNvtx = 0;
    ...
    TH2D * lpt_bpt = new TH2D("lpt_bpt", "lpt_bpt", 50, 0., 200., 50, 0., 200.);
    TH2D * lpt_beta = new TH2D("lpt_beta", "lpt_beta", 100, -5., 5., 50, 0., 200.); //X: Eta Y: Pt
    TH2D * bpt_beta = new TH2D("bpt_beta", "bpt_beta", 100, -5., 5., 50, 0., 200.); //X: Eta Y: Pt
    TH3D * bpt_beta_lpt = new TH3D("bpt_beta_lpt", "bpt_beta_lpt", 100, -5., 5., 50, 0., 200., 50, 0., 200.); //X: Eta Y: Pt Z:Pt
#ifdef LPTW
    double lptw = 1.;
    TH1D * lptw_cosTheta = new TH1D("lptw_cosTheta", "lptw_cosTheta", 100, -1., 1.);
    lptw_cosTheta->Sumw2();
    TH1D * bptw_cosTheta = new TH1D("bptw_cosTheta", "bptw_cosTheta", 100, -1., 1.);
    bptw_cosTheta->Sumw2();
    TH1D * betaw_cosTheta = new TH1D("betaw_cosTheta", "betaw_cosTheta", 100, -1., 1.);
    betaw_cosTheta->Sumw2();
    TH1D * lbptw_cosTheta = new TH1D("lbptw_cosTheta", "lbptw_cosTheta", 100, -1., 1.);
    lbptw_cosTheta->Sumw2();
    TH1D * allw_cosTheta = new TH1D("allw_cosTheta", "allw_cosTheta", 100, -1., 1.);
    allw_cosTheta->Sumw2();
    TH1D * lptbetaw_cosTheta = new TH1D("lptbetaw_cosTheta", "lptbetaw_cosTheta", 100, -1., 1.);
    lptbetaw_cosTheta->Sumw2();
    TH1D * bptetaw_cosTheta = new TH1D("bptetaw_cosTheta", "bptetaw_cosTheta", 100, -1., 1.);
    bptetaw_cosTheta->Sumw2();
    TFile * flptw = 0;
    if (dirName == "TreesEle") {
        if (e_mtw)
            flptw = TFile::Open("TreesEle_weights.root");
        else
            flptw = TFile::Open("TreesEle_weights.root");
    } else if (dirName == "TreesMu")
        flptw = TFile::Open("TreesMu_weights.root");
    TH1D * hlptw = (TH1D*) flptw->Get("lptw");
    TH1D * hbptw = (TH1D*) flptw->Get("bptw");
    TH1D * hbetaw = (TH1D*) flptw->Get("betaw");
    TH2D * hlbptw = (TH2D*) flptw->Get("lbptw");
    TH2D * hlptbetaw = (TH2D*) flptw->Get("lptbetaw");
    TH2D * hbptbetaw = (TH2D*) flptw->Get("bptbetaw");
    TH3D * hlbptbetaw = (TH3D*) flptw->Get("lbptbetaw");
    JetHists bptw_bpt("bptw_bpt", 2);
    JetHists betaw_beta("betaw_beta", 2);
    MuonHists lptw_l("lptw_Lepton", 3);
    ...
    for (unsigned int fNumber = 0; fNumber < inputFileNames.size(); fNumber++) {
        ...
        for (int eventNumber = 0; eventNumber < myMuonTree->fChain->GetEntriesFast(); eventNumber++) {
            ...
            lpt_beta->Fill(myMuonTree->bJetEta, myMuonTree->leptonPt, lumiWeight3D);
            lpt_bpt->Fill(myMuonTree->bJetPt, myMuonTree->leptonPt, lumiWeight3D);
            bpt_beta->Fill(myMuonTree->bJetEta, myMuonTree->bJetPt, lumiWeight3D);
            bpt_beta_lpt->Fill(myMuonTree->bJetEta, myMuonTree->bJetPt, myMuonTree->leptonPt, lumiWeight3D);
            ...
            if (ttDecayMode == "") {
                if (myLeptonicTop.hasNeutrinoSolution()) {
                    ...
                    //                    dphilmet_cosTheta->Fill(myLeptonicTop.cosThetaStar(), myMuonTree->metPhi - myMuonTree->leptonPhi);
                    //                    lpt_cosTheta->Fill(myLeptonicTop.cosThetaStar(), myMuonTree->leptonPt);
            ...
            EventFlavor->Fill(myMuonTree->eventFlavour, lumiWeight3D);
            int bin = -1;
#ifdef LPTW
            //            cout << myMuonTree->leptonPt << endl;
            bin = hlptw->GetXaxis()->FindBin(myMuonTree->leptonPt);
            //            cout << bin << endl;
            //            cout << hlptw->GetBinContent(bin) << endl;
            //            cout << lptw << endl;
            lptw = hlptw->GetBinContent(bin);
            //            cout << lptw << endl;
            lptw_cosTheta->Fill(myLeptonicTop.cosThetaStar(0), lptw * lumiWeight3D);
            lptw_l.Fill(myEvent_tmp.Dmuons, myEvent_tmp.Dmuons.size(), lptw * lumiWeight3D);

            bin = hbptw->GetXaxis()->FindBin(myMuonTree->bJetPt);
            double bptw = hbptw->GetBinContent(bin);
            bptw_cosTheta->Fill(myLeptonicTop.cosThetaStar(0), bptw * lumiWeight3D);
            bptw_bpt.FillPFJets(myEvent_tmp.BPFJets, myEvent_tmp.BPFJets.size(), myEvent_tmp.BPFJets.size(), false, bptw * lumiWeight3D);

            bin = hbetaw->GetXaxis()->FindBin(myMuonTree->bJetEta);
            double betaw = hbetaw->GetBinContent(bin);
            betaw_cosTheta->Fill(myLeptonicTop.cosThetaStar(0), betaw * lumiWeight3D);
            betaw_beta.FillPFJets(myEvent_tmp.BPFJets, myEvent_tmp.BPFJets.size(), myEvent_tmp.BPFJets.size(), false, betaw * lumiWeight3D);


            bin = hlbptw->FindBin(myMuonTree->bJetPt, myMuonTree->leptonPt);
            lbptw_cosTheta->Fill(myLeptonicTop.cosThetaStar(0), (hlbptw->GetBinContent(bin) * lumiWeight3D));

            bin = hlptbetaw->FindBin(myMuonTree->bJetEta, myMuonTree->leptonPt);
            lptbetaw_cosTheta->Fill(myLeptonicTop.cosThetaStar(0), (hlptbetaw->GetBinContent(bin) * lumiWeight3D));

            bin = hbptbetaw->FindBin(myMuonTree->bJetEta, myMuonTree->bJetPt);
            bptetaw_cosTheta->Fill(myLeptonicTop.cosThetaStar(0), (hbptbetaw->GetBinContent(bin) * lumiWeight3D));

            bin = hlbptbetaw->FindBin(myMuonTree->bJetEta, myMuonTree->bJetPt, myMuonTree->leptonPt);
            allw_cosTheta->Fill(myLeptonicTop.cosThetaStar(0), (hlbptbetaw->GetBinContent(bin) * lumiWeight3D));
#endif
        }
...
#ifdef LPTW
    lptw_cosTheta->Write();
    lptw_l.WriteAll(fout);
    bptw_cosTheta->Write();
    bptw_bpt.WriteAll(fout);
    betaw_cosTheta->Write();
    betaw_beta.WriteAll(fout);
    bptetaw_cosTheta->Write();
    lptbetaw_cosTheta->Write();
    lptbetaw_cosTheta->Write();
    allw_cosTheta->Write();
#endif
    lpt_beta->Write();
