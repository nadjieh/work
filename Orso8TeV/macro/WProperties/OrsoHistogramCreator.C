...
double GetEtaWeight(TH1D* in, double eta, double realW = false) {
    int bin = in->GetXaxis()->FindBin(eta);
    if (realW)
        return in->GetBinContent(bin);
    return 1.;
}
...
int main(int argc, char** argv) {
    ...
    WtemplateHistograms myWTemplate("WProperties", false, nBinY);
    if (!isStack)
        myWTemplate.SetErrors(true);
    WtemplateHistograms myWTemplateReal("WPropertiesReal", false, nBinY);
    if (!isStack)
        myWTemplateReal.SetErrors(true);
    ...
    for (unsigned int fNumber = 0; fNumber < inputFileNames.size(); fNumber++) {
        ...
        for (int eventNumber = 0; eventNumber < myMuonTree->fChain->GetEntriesFast(); eventNumber++) {
            ...
            if (ttDecayMode == "") {
                ...
                myWTemplate.Fill(myLeptonicTop, lumiWeight3D, 0, genSingleTop);
                lumiWeight3D = lumiWeight3D * GetEtaWeight(allWEta, myLeptonicTop.W().Eta());
    ...
    myWTemplateReal.Write(fout);
    myWTemplate.Write(fout);
    EventFlavor->Write();
    WEtaAll->Write();
    WEtaReal->Write();
    fout->Write();
