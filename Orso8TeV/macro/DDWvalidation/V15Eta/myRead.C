...
string settitles(string s){
	...
	if(s == string("allW_BJet_Pt"))
		return string("p_{T}^{b-jets} (GeV)");
	if(s == string("allW_Muon_Pt"))
		return string("p_{T}^{l}");
	if(s == string("EtaFwD_allWcosTheta"))
		return string("cos(#theta_{l}*)");
	if(s == string("allW_FwD_Eta"))
		return string("#eta_{jet}^{FwD}");
	if(s == string("allW_Jet_Pt"))
		return string("p_{T}^{jets} (GeV)");
	if(s == string("allW_finalMT"))
		return string("m_{T}^{W} (GeV)");
	if(s == string("allW_Met_Pt"))
		return string("E_{T}^{miss} (GeV)");
	if(s == string("Default_allW_topMass"))
		return string("m_{lb#nu} (GeV)");
	return string("cos(#theta_{l}*)");
}
string setYtitles(string s){
	string ret=string("Events @ 19.8 fb^{-1}");
	if(s == string("allW_Jet_Pt"))
		ret = string("Jets @ 19.8 fb^{-1}");
	return ret;
}
string getlegend(){
  ...
  leg+="   leg->Draw();\n";
  leg+="   TLatex *   tex = new TLatex(0.27,0.9684385,\"CMS preliminary, 19.8 fb^{-1} at #sqrt{s} = 8 TeV\");\n";
  leg+="   tex->SetNDC();\n";
...
void doJob(){
	...
	myRead("MtopOutWindowTrue_allWcosTheta","MtopOutWindow_True.C");
	myRead("MtopOutWindow_allWcosTheta","MtopOutWindow.C");
	myRead("MtopInWindowTrue_allWcosTheta","MtopInWindow_True.C");
	myRead("MtopInWindow_allWcosTheta","MtopInWindow.C");
	
	myRead("EtaFwDTrue_allWcosTheta","Eta_True.C");
	myRead("EtaFwD_allWcosTheta","Eta.C");
	myRead("antiEtaFwDTrue_allWcosTheta","antiEta_True.C");
	myRead("antiEtaFwD_allWcosTheta","antiEta.C");	
