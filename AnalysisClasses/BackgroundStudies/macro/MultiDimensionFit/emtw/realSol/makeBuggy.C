...
void doJob(){
	...
	for(unsigned int i = 0; i<Enames.size(); i++){
		cout<<"  - "<<Enames[i]<<endl;
		GetCorr("goodTT/TreesEle_"+Enames[i]+"_TTBar_RW.root",3);
		nm.push_back(GetEntries("goodTT/TreesEle_"+Enames[i]+"_TTBar_RW.root"));
		nM+= nm[nm.size()-1];
	} 
	cout<<"  - t"<<endl;
	GetCorr("goodTT/TreesEle_TChannel_RW.root",2);
	cout<<"  - tbar"<<endl;
    nm.push_back(GetEntries("goodTT/TreesEle_TChannel_RW.root"));
    nM+= nm[nm.size()-1];
	GetCorr("goodTT/TreesEle_TbarChannel_RW.root",2);
    nm.push_back(GetEntries("goodTT/TreesEle_TbarChannel_RW.root"));
    nM+= nm[nm.size()-1];
