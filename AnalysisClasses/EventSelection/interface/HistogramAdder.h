...
public:
    ...
    //
        bool doIwantThis(TString name) {
            int pos = string(name).find("allW");
            if (pos == 0)
                return true;
            if (name == "Default_allW" || name == "EtaFwD_allW" || name == "EtaFwDTrue_allW" || name == "DefaultTrue_allW")
                return true;
            return false;
        }

//    bool doIwantThis(TString name) {
//        return (name == "DefaultTrue_allW" );
//    }
                ...
                for (unsigned int e = 0; e < files.size(); e++) {
                    ...
                    if (structure.at(q).content.at(w) == string(structure.at(q).mainName + "cosTheta")) {
                        ...
                        if (doProjection && (string(files.at(e)->GetName()).find("qcd") == 0 || string(files.at(e)->GetName()).find("qcd") >
                            ...
                            TH1D* h2d = 0;
                            if (d->Get(string(structure.at(q).mainName + "cosTheta2D").c_str()))
                                h2d = ((TH2D*) d->Get(string(structure.at(q).mainName + "cosTheta2D").c_str()))->ProjectionY();
                            //                            h2d->Sumw2();
                            if (h2d != NULL) {
                    ...
                    if (structure.at(q).content.at(w) == string(structure.at(q).mainName + "cosTheta_MET")) {
                        if (doProjection && (string(files.at(e)->GetName()).find("qcd") == 0 || string(files.at(e)->GetName()).find("qcd") >
                            ...
                            TH1D * h2d = 0;
                            if (d->Get(string(structure.at(q).mainName + "cosTheta2D_MET").c_str()))
                                h2d = ((TH2D*) d->Get(string(structure.at(q).mainName + "cosTheta2D_MET").c_str()))->ProjectionY();
                            //                            if(h2d->GetNbinsX() != h->GetNbinsX())
                            //                                h2d->Rebin(int(h2d->GetNbinsX()/h->GetNbinsX()));
                            if (h2d != NULL) {
                                h2d->Scale(weights.at(e));
                    ...
                    if (Unc.size() != 0)
                        if (Unc[e] > 0) {
                            for (int nBins = 0; nBins < h->GetXaxis()->GetNbins(); nBins++) {
                                double t1_2 = pow(h->GetBinError(nBins + 1), 2);
                                double t2_2 = pow((Unc[e] * h->GetBinContent(nBins + 1)), 2);
                                h->SetBinError(nBins + 1, sqrt(t1_2 + t2_2));
                ...
                if (Unc.size() != 0)
                    if (Unc[e] > 0) {
                        for (int nBins = 0; nBins < cf->GetXaxis()->GetNbins(); nBins++) {
                            double t1_2 = pow(cf->GetBinError(nBins + 1), 2);
                            double t2_2 = pow((Unc[e] * cf->GetBinContent(nBins + 1)), 2);
                            cf->SetBinError(nBins + 1, sqrt(t1_2 + t2_2));
